...
public:
    ...
    nlohmann::json to_json() const {
        nlohmann::json j;
        j["C"] = C.to_json(); // Assuming layeredCircuit has a to_json() method
        // Add other relevant data members here
        return j;
    }

    static prover from_json(const nlohmann::json &j) {
        prover p;
        p.C = layeredCircuit::from_json(j["C"]); // Assuming layeredCircuit has a from_json() method
        // p.val = j["val"].get<std::vector<std::vector<F>>>();
        // Deserialize other relevant data members here
        return p;
    }
