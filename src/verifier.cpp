...
bool verifier::verify() {
    ...
    poly_v = std::make_unique<hyrax_bls12_381::polyVerifier>(p -> commitInput(gens), gens); // Prover commit the input layer
    fprintf(stderr, "p -> commitInput(gens);\n");
...
bool verifier::verifyInnerLayers() {
    ...
    for (u8 i = C.size - 1; i; --i) {
        ...
        fprintf(stderr, "p -> sumcheckInit(alpha, beta);\n");
        total_timer.start();
        total_slow_timer.start();
        ...
        if (cur.ty == layerType::DOT_PROD) {
            ...
            fprintf(stderr, "p -> sumcheckDotProdInitPhase1();\n");
        }
        else {
            p -> sumcheckInitPhase1(relu_rou);
            fprintf(stderr, "p -> sumcheckInitPhase1(relu_rou);\n");
        }
        ...
        if (cur.ty == layerType::DOT_PROD){
            ...
            fprintf(stderr, "p -> sumcheckDotProdFinalize1(previousRandom, final_claim_u1);\n");
        }
        else {
            p -> sumcheckFinalize1(previousRandom, final_claim_u0[i], final_claim_u1);
            fprintf(stderr, "p -> sumcheckFinalize1(previousRandom, final_claim_u0[i], final_claim_u1);\n");
        }
        ...
        if (cur.need_phase2) {
            ...
            fprintf(stderr, "p -> sumcheckFinalize2(previousRandom, final_claim_v0[i], final_claim_v1);\n");
...
bool verifier::verifyFirstLayer() {
    ...
    fprintf(stderr, "p -> sumcheckLiuFinalize(previousRandom, eval_in);\n");
